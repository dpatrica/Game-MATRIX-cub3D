...
static t_map	ft_valyas(t_map param, int i, int j, int flaglen)
{
	...
	{
		if (ft_rhr("023ACMYPXHDZNSWE", param.g_map[i][j]))
		{
			...
			if (!(ft_rhr("1023ACMYPXHDZNSWE", param.g_map[i][j - 1])) ||\
			!(ft_rhr("1023ACMYPXHDZNSWE", param.g_map[i - 1][j - 1])) ||\
			!(ft_rhr("1023ACMYPXHDZNSWE", param.g_map[i - 1][j])) ||\
			!(ft_rhr("1023ACMYPXHDZNSWE", param.g_map[i - 1][j + 1])) ||\
			!(ft_rhr("1023ACMYPXHDZNSWE", param.g_map[i][j + 1])) ||\
			!(ft_rhr("1023ACMYPXHDZNSWE", param.g_map[i + 1][j + 1])) ||\
			!(ft_rhr("1023ACMYPXHDZNSWE", param.g_map[i + 1][j])) ||\
			!(ft_rhr("1023ACMYPXHDZNSWE", param.g_map[i + 1][j - 1])))
				param.valid = MAP_NO_VALID;
